...
function LanguageSettings() {
  ...
  // Two-factor account-security flow state. "reset" and "regenerate" both
  // start with a password-confirm modal (each invalidates something that
  // already exists); "enable" skips straight to TotpSetupForm since there's
  // nothing yet to lose by starting fresh — the same reasoning the
  // mandatory-during-login setup path already relies on.
  const [twoFaAction, setTwoFaAction] = useState(null) // null | 'enable-setup' | 'reset-confirm' | 'reset-setup' | 'regenerate-confirm'
  ...
  // Same completion shape as a reset's setup step — reused directly rather
  // than duplicated, since both end at "a confirmed setup just happened,
  // show the codes and refresh admin state."
  const handleEnableSetupComplete = handleResetSetupComplete
  ...
  return (
    <div>
      ...
      <div className="content">
        ...
        <p style={{ fontSize: 12.5, color: 'var(--slate)', marginTop: 0, marginBottom: 10, maxWidth: 480 }}>
          {admin.totp_enabled
            ? 'If you lose access to your authenticator app, reset it below — your password is required first.'
            : admin.totp_required
              ? "Required for your account — you'll be asked to set it up on your next login."
              : 'Adds a second step when signing in, on top of your password. Optional for now — turn it on for extra security.'}
        </p>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {admin.totp_enabled ? (
            ...
            <button type="button" className="btn btn-primary" onClick={() => setTwoFaAction('enable-setup')}>
      ...
      {twoFaAction === 'enable-setup' && (
        <Modal title="Set up two-factor authentication" onClose={() => setTwoFaAction(null)}>
          <TotpSetupForm onComplete={handleEnableSetupComplete} onCancel={() => setTwoFaAction(null)} />
        </Modal>
      )}
