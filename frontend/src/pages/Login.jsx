...
// Two-factor authentication is enforced per-account (see admin.js's
// totp_required) — an account with it turned on gets routed here instead
// of straight to a session, so this is a small state machine rather than a
// single form:
//   'credentials' -> (2FA already configured)      -> 'verify' -> dashboard
//   'credentials' -> (2FA required, not set up yet) -> 'setup' -> 'backup-codes' -> dashboard
//   'credentials' -> (2FA not required)             -> dashboard directly
function Login() {
  ...
  async function handleCredentialsSubmit(e) {
    ...
    try {
      ...
      else if (result.requires_2fa) setStep('verify')
      else finishLogin()
    } catch (err) {
      ...
      <div className="login-screen login-screen-branded">
        <div className="login-card" style={{ maxWidth: 420 }}>
          ...
          <p style={{ fontSize: 12.5, color: 'var(--slate)', marginTop: -8, marginBottom: 16 }}>
            Required for your account — this only takes a minute.
          </p>
          <TotpSetupForm
            onComplete={(result) => {
