...
// Two-factor authentication is enforced per-account, not globally: an
// admin with totp_enabled or totp_required set never gets a real session
// on password alone — req.session.adminId is set only in /2fa/verify or
// /2fa/setup/confirm, once the second factor (or a fresh setup) is
// actually satisfied. An account with neither flag set logs in exactly as
// it always has. See schema.sql's totp_required comment for why this is
// staged per-account rather than unconditional for every dashboard
// account the moment this code deploys.
router.post(
  "/login",
  asyncHandler(async (req, res) => {
    ...
    const { rows } = await pool.query(
      `SELECT a.id, a.email, a.password_hash, a.timezone, a.totp_enabled, a.totp_required, a.business_id
       FROM admins a
    ...
    // Fires on password verification alone, before any 2FA step below —
    // deliberately earlier than "login fully complete", so a stolen
    // password alone still alerts the real owner even if the attacker
    ...
    // A secret already exists and is confirmed — always verify against it,
    // regardless of totp_required (that flag only controls whether an
    // account with *no* secret yet gets routed into setup; it was never
    // meant to let a since-disabled rollout skip a check an account has
    // already turned on for itself from Settings).
    if (admin.totp_enabled) {
      req.session.pending2faAdminId = admin.id;
      return res.json({ requires_2fa: true });
    }
    // Not yet set up, and this account is enrolled in the rollout — route
    // into the mandatory setup flow rather than blocking the account out
    // entirely. totp_required defaults false (see schema.sql), so an
    // ordinary account is never affected until deliberately flipped.
    ...
    res.json(await loadAdminSummary(admin.id));
  })
);
  ...
  asyncHandler(async (req, res) => {
    const { rows } = await pool.query(
      `SELECT a.id, a.email, a.role, a.language, a.timezone, a.push_notify_other, a.totp_enabled, a.totp_enabled_at,
              a.totp_required, a.business_id, b.business_name, b.created_at AS trial_started_at, b.logo_url,
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const { rows: adminRows } = await client.query(
        "INSERT INTO admins (email, password_hash, business_id, role, timezone) VALUES ($1, $2, $3, 'owner', $4) RETURNING id, email, role, timezone, totp_required",
      ...
      // Same rule as /login: a brand-new admin row's totp_required defaults
      // false (see schema.sql), so a normal signup gets a real session
      // immediately, exactly as it always has, until the rollout is turned
      // on. At that point flipping totp_required's own column default to
      // true makes every new signup go straight into mandatory setup with
      // no code change here — see /login's own comment for the full
      // reasoning on why this is gated per-account rather than unconditional.
      if (admin.totp_required) {
        req.session.pendingSetup2faAdminId = admin.id;
        return res.status(201).json({ requires_2fa_setup: true });
      }
